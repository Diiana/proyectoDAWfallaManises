...
var control_comision_list = function(path) {
    //contexto privado

    var prefijo_div = "#comision_list ";

    function cargaBotoneraMantenimiento() {
        var botonera = [
            {"class": "btn btn-mini action05", "icon": "", "text": "Falleros"}
    ...
    function cargaComision(id) {

        var censo = objeto('censo', path);
        var censoView = vista(censo, path);

        $('#indexContenidoJsp').empty();
        $('#indexContenido').empty().append(censoView.getEmptyList());

        var censoControl = control_censo_list(path);
        censoControl.inicia(censoView, 1, null, null, 10, null, null, null, null, "id_comision", "equals", id);
        return false;
